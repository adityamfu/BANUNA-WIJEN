...
const ppp = document.querySelector(".mainContainer");

if (await specifiedTakeData("admin-login", localStorage.getItem("token"), "") !== true) {
    ppp.remove(ppp)
    performLogin();
} else {
    ...
    document.querySelectorAll(".items").forEach(icon => {
        icon.addEventListener("click", () => {
            ...
            const sectionMap = {
                ...
                "Logout": async () => {
                    universalDataFunction("update", "admin-login", localStorage.getItem("token"), "")
                    Swal.fire({
                        icon: 'success',
    ...
    async function prepareTransaction() {
        ...
        if (DataTransaksiArray.length > 0) {
            DataTransaksiArray.forEach(transaction => {
                ...
                <div class="traCards" id="traCards-${transaction.IDTransaksi}">
                    <div>
            ...
            TandaiButtons.forEach(button => {
                button.addEventListener("click", function () {
                    ...
                    universalDataFunction("update", "transactions", `${strukId}.Status`, "selesai");
                    Swal.fire({
                        icon: 'success',
