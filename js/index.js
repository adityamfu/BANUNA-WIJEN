...
import { universalTakeData, database, generateRandomAlphanumeric, generateCurrentTime } from './data-controller.js';
...
async function setStoreDataUi() {
    ...
    document.getElementById("KontakNav").href = `https://wa.me/${data.storeMainContact}`;
    document.getElementById("InstagramToko").closest("a").href = `https://instagram.com/${data.storeIg}`
    document.getElementById("FacebookToko").closest("a").href = `https://facebook.com/@${data.storeFb}`
    document.getElementById("WhatsappToko").closest("a").href = `https://wa.me/${data.storeMainContact}`
    document.getElementById("FootIG").href = `https://instagram.com/${data.storeIg}`
    document.getElementById("FootFb").href = `https://facebook.com/@${data.storeFb}`
    document.getElementById("FootWa").href = `https://wa.me/${data.storeMainContact}`
...
async function displayFeedback() {
    try {
        ...
        if (feedData && typeof feedData === 'object') {
            ...
            const validFeedbackKeys = feedbackKeys.filter(key => feedData[key].status !== 'unchecked'); //valid checking 
            ...
            feedBullets.innerHTML = validFeedbackKeys.map(() => '<div class="feedback-bullet"></div>').join('');
            ...
            const updateFeedback = () => {
                ...
                feedbackDetails.innerHTML = `
                    <div class="feedbackThing">
                        <div class="feedname"><b>${feed.feedbackName || 'Anonymous'}</b></div>
                        <div class="feedText">${feed.feedbackText || 'No feedback provided.'}</div>
                    </div>
                `;
...
async function displayProducts() {
    try {
        ...
        if (prodData && typeof prodData === 'object') {
            const productCards = Object.keys(prodData).map(key => {
                const product = prodData[key];
                return `
                <div class="Card">
                    <div class="card-image">
                        <img src="${product.productImage || './images/default.png'}" alt="${product.productName}" />
                    </div>
                    <div class="Frame2812">
                        <div class="Frame2657">
                            <div class="WijenOriginal">${product.productName}</div>
                            <div class="PisangDenganTaburanWijen">
                                ${product.productDesc}
                            </div>
                        </div>
                        <div class="Frame2813">
                            <div class="productPrice">Rp${product.productPrice}</div>
...
document.getElementById("KirimFeedback").addEventListener('click', async (e) => {
    Swal.fire({
        title: 'Masukkan Nama',
        input: 'text',
